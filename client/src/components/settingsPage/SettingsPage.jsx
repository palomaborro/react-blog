import React from "react";
import SideBar from "../sidebar/SideBar";
import "./SettingsPage.scss";

export default function SettingsPage() {
  return (
    <div className="SettingsPage">
      <div className="SettingsPage__container">
        <div className="SettingsPage__container__title">
          <span className="SettingsPage__container__title__update">
            Update your account
          </span>
          <span className="SettingsPage__container__title__delete">
            Delete account
          </span>
        </div>
        <form className="SettingsPage__form">
          <label>Profile picture</label>
          <div className="SettingsPage__form__picture">
            <img
              className='SettingsPage__form__picture__img'
              src="data:image/png;base64,"
              alt=""
            />
            <label htmlFor="fileInput">
              <i className="SettingsPage__form__picture__icon fas fa-user"></i>
            </label>
            <input
              className="SettingsPage__form__picture__input"
              type="file"
              id="fileInput"
            />
          </div>
          <label>User name</label>
          <input type="text" placeholder="Rehnn"></input>
          <label>Email</label>
          <input type="email" placeholder="rehnn@gmail.com"></input>
          <label>Password</label>
          <input type="password"></input>
          <button className='SettingsPage__form__button'>Update</button>
        </form>
      </div>
      <SideBar />
    </div>
  );
}
